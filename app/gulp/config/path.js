...
export const path = {
    ...
    resolveDest(path) {
        return {
            src: app.isWP && app.forPlugin ? [...path.src.php, ...path.src.plug] : app.isWP ?
                path.src.php : app.forPlugin ?
                path.src.plug : path.src.html,
            ...(Object.keys(path.prod).length !== 0 ? {
                dest: app.isWP && app.forPlugin ? [path.prod.php, path.prod.plug] : app.isWP ?
                    path.prod.php : app.forPlugin ?
                    path.prod.plug : path.prod.html,
            } : {}),
        };
    },
